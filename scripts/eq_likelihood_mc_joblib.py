...
# define function in script so as not to have to pass all the variables
# or abstract things too much
def index_loop(df, index):
    ...
    for t in time_window:
        ss[t] = ( eqs.get_prob_above_val_in_window(eq_series, min_mag, t) 
                * slr_dip_frac )
    return ss

# 
series_list = Parallel(n_jobs=-2) (delayed( index_loop)(slr_6, ii) 
                                    for ii in mc_index)

for ii, ser in enumerate(series_list):
    ind = ser.index
    slr_6.iloc[ii][list(ind)] = ser.values
